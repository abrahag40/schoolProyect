...
4. **Hablar desde el sombrero correspondiente** (SME Educación, PM, PO, Scrum Master, UX Research, **UX Architect, Interaction Designer, Product Designer** —definidos en [docs/ux/sombreros-de-diseno.md](docs/ux/sombreros-de-diseno.md); sustituyen al genérico «UX/UI»—, Arquitecto, Dev, QA, Seguridad & Compliance, Analista de Mercado), cada uno con seniority y solo dentro de su dominio. El disenso entre sombreros se documenta, no se esconde. El CEO es sponsor: decide en gates.
...
- **Análisis del prospecto OFASE v1.0** (dos guías de una plataforma deportiva en producción, leídas como evidencia y no como fuente de la verdad: diez defectos ajenos convertidos en reglas, cuatro cosas que conviene robar, y la verificación en código de que ADR-011 ya nos cubría del peor de ellos): https://claude.ai/code/artifact/fe1baabe-f1ec-4af1-9db0-7d1265a6e291
- **Épica E10 · Vertical deportiva como add-on v1.0** (D21 APROBADA: 34 requisitos `AZ-A12.#` con MoSCoW y origen trazado, ADR-014 con las cuatro condiciones que definen un add-on, gate de arranque de 4 condiciones, y el disenso documentado sobre `AZ-A12.7`). Documento vivo en [docs/producto/AZ-A12-vertical-deportiva.md](docs/producto/AZ-A12-vertical-deportiva.md): https://claude.ai/code/artifact/0797c219-c9f9-4c79-a235-1ae214ccbc54
- **Los tres sombreros de diseño v1.0** (11-sep-2026: UX Architect · Interaction Designer · Product Designer, cada uno dueño de un plano de Garrett —estructura, esqueleto, resultado—; las seis observaciones del CEO sobre el frontend verificadas en código; propuestas UXA-1..4, IxD-1..4, PD-1..4 con su estándar; la secuencia de gates hasta los mocks. **§72** registrada: una escuela nace en ZaharDev, la consola nunca ofrece «agregar escuela»). Documento vivo en [docs/ux/sombreros-de-diseno.md](docs/ux/sombreros-de-diseno.md): https://claude.ai/code/artifact/c9edeb9a-f72d-4594-a156-c454aad0f615
...
- **Sprint 7 CERRADO con ceremonia completa** (rama `sprint-7-diseno-layout`, tag `v0.8.0`). Acta en [docs/sprints/acta-cierre-S7.md](docs/sprints/acta-cierre-S7.md), backlog en [docs/sprints/S7-diseno-y-layout.md](docs/sprints/S7-diseno-y-layout.md). **Aprovechamiento del ancho: 61 % → 100 %**; contenedores con ancho propio 16 → 0. Entregado el `Must` completo (tokens de layout, `ArmazonPanel`, `Rejilla`/`Lectura`, 6 pantallas migradas y 8 pruebas de navegador con mordida verificada). **NO entregado:** `AZ-D1.6` solo en el Panel y `AZ-D1.7` sin hacer — advertidos por escrito al ampliar el alcance. **Desviación declarada:** Tailwind 4 y el puente a Metronic entraron fuera de alcance (ADR-012, §67); no se revierten porque están probados. Decisiones §64–§67. 276 pruebas del API, 23 de aislamiento, **22 de navegador** (eran 14), ensayo de despliegue superado.
- **Sprint 8 CERRADO con ceremonia completa y ACEPTADO por el CEO** (rama `sprint-8-metronic`, tag `v0.9.0`, mergeada a `main` el 6-sep-2026). Acta en [docs/sprints/acta-cierre-S8.md](docs/sprints/acta-cierre-S8.md), backlog en [docs/sprints/S8-metronic.md](docs/sprints/S8-metronic.md), inventario en [docs/adr/inventario-metronic.md](docs/adr/inventario-metronic.md). **El frontend entero corre sobre Metronic**: 110 archivos adoptados (14,470 líneas), 8 pantallas reconstruidas —las 6 del panel más login y 404—, Cobranza sobre el `DataGrid` con las 6 columnas de un estudio propio, y el riel plegable de 80 px. **Los 9 `Must` completos** (segundo sprint desde el S2 que entrega su alcance obligatorio íntegro). Decisiones nuevas §68–§69, ADR-013 (deroga la sección «NO se adopta su armazón» de ADR-012), D19 y D20 registradas con su riesgo. **276 pruebas del API, 23 de aislamiento, 25 de navegador, 16 de tokens**, medidas SIN caché de turbo. Ensayo de despliegue superado y desplegado a staging.
...
- **§70/§71** — «Add-on» no es un prefijo: son CUATRO condiciones (esquema propio, dependencia unidireccional con gate, contratación en `plataforma`, navegación por rol × módulo; ADR-014). Y el `Must` de un add-on es del MÓDULO, nunca del MVP.

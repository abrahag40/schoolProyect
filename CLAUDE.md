...
- **Sprint activo: NINGUNO.** El S6 cerró el 2-sep (`v0.7.0`). Entre el 2 y el 4-sep hubo un **intervalo fuera de sprint** (despliegue + 4 defectos), cerrado con `v0.7.1` y documentado como desviación de §8 en [docs/sprints/intervalo-post-S6-despliegue.md](docs/sprints/intervalo-post-S6-despliegue.md). **La regla de intercambio del C3 YA SE PUEDE EVALUAR y su resultado es que el S7 CEDE EL TURNO:** la condición 1 (nube + staging) ahora se cumple, pero la 2 (ADR-007, proveedor de pagos — el archivo ni existe) y la 3 (cuenta de comercio con sandbox) siguen en NO. Por la regla escrita por adelantado, procede el **S8 Comunicación I**, sin junta. **PERO el CEO pidió un sprint de UI/UX** (hallazgo del 4-sep: no hay capa de layout responsivo), que NO está en el plan: entra por el mecanismo de 3 salidas de §8 y es decisión suya en gate.

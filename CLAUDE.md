...
- **Sprint 8 CERRADO con ceremonia completa** (rama `sprint-8-metronic`, tag `v0.9.0`). Acta en [docs/sprints/acta-cierre-S8.md](docs/sprints/acta-cierre-S8.md), backlog en [docs/sprints/S8-metronic.md](docs/sprints/S8-metronic.md), inventario en [docs/adr/inventario-metronic.md](docs/adr/inventario-metronic.md). **El frontend entero corre sobre Metronic**: 110 archivos adoptados (14,470 líneas), 8 pantallas reconstruidas —las 6 del panel más login y 404—, Cobranza sobre el `DataGrid` con las 6 columnas de un estudio propio, y el riel plegable de 80 px. **Los 9 `Must` completos** (segundo sprint desde el S2 que entrega su alcance obligatorio íntegro). Decisiones nuevas §68–§69, ADR-013 (deroga la sección «NO se adopta su armazón» de ADR-012), D19 y D20 registradas con su riesgo. **276 pruebas del API, 23 de aislamiento, 25 de navegador, 16 de tokens**, medidas SIN caché de turbo. Ensayo de despliegue superado y desplegado a staging.
  ...
  - **NO entregado:** `AZ-D2.10` es **PARCIAL** —los formularios se rehicieron pero sin `react-hook-form` ni validación con zod, que era la razón de adoptar su `Form`— y `AZ-D2.12` (qué hacer con `packages/ui`) es decisión del CEO.

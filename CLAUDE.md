...
- **Sprint activo: ninguno. SPRINT 7 propuesto — "Dinero real"** (pasarela, conciliación y POC Facturama), **CONDICIONADO** por la regla de intercambio del cambio C3: si no existen las tres cuentas de nube, el proveedor decidido (ADR-007) y la cuenta de comercio, el S7 cede el turno al S8 (Comunicación I) sin junta.
  ...
  - **Impedimento del S0 CERRADO el 4-sep-2026 — Azahar está desplegado.** Siete sprints y 8 migraciones después, hay staging real: **API** https://azahar-api.onrender.com (Render, Docker, Oregon) · **Web** https://azahar-web-neon.vercel.app (Vercel; el sufijo `-neon` es aleatorio y NO tiene relación con la base) · **DB** Neon `jolly-lake-32570910`, PG 16. Verificado de extremo a extremo desde el navegador: la web cruza a Render, Render cruza a Neon y vuelve. Estado vivo y los cuatro fallos que costó, en [docs/operacion/INFRA.md](docs/operacion/INFRA.md).
    ...
    - **PENDIENTE — sembrar staging.** La base tiene las 8 migraciones pero **cero datos**: se puede abrir la web, no se puede entrar. Bloqueo posible: `FORCE ROW LEVEL SECURITY` (§3) alcanza también al dueño de la tabla, y en Neon el dueño no es superusuario. El arranque ya lo mide y lo dice en el log.
